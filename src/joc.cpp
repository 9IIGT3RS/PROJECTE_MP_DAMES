...
bool Joc::actualitzaModusReplay(int mousePosX, int mousePosY, bool mouseStatus) {
    ...
    if (tempsActual - m_tempsUltimMoviment > 1000) { // 1 segon entre moviments
        ...
        if (!m_cua.esBuida()) {
            ...
            std::cout << "No queden més moviments a reproduir." << std::endl;
            m_partidaAcabada = true;
            return true;
        }
    }
...
void Joc::executaMoviment(const Posicio& desti) {
    ...
    // Comprovem si hi havia captures obligatòries ABANS de moure
    ...
    if (m_tauler.mouFitxa(m_posicioSeleccionada, desti)) {
        ...
        if (hiHaviaCapturesPossibles) {
            ...
            // Si el moviment no és una captura o no és la millor captura
            if (movimentExecutat.getNumCaptures() == 0 ||
                ...
                std::cout << "BUFAT! No has fet la captura obligatòria o màxima!" << std::endl;
